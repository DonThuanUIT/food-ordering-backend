...
*   `[NEW]` [V9__add_is_open_to_shop_settings.sql](file:///d:/food-ordering-backend/backend/src/main/resources/db/migration/V9__add_is_open_to_shop_settings.sql): Script bổ sung cột trạng thái mở cửa tức thời `is_open` còn thiếu trên bảng `shop_settings` để đồng bộ với cấu hình thực thể JPA và chuyển dữ liệu tương ứng từ bảng `shops`.
